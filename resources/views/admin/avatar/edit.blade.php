...
<body>
    <x-app-layout>
        <div class="max-w-3xl mx-auto pt-7">
            <form action="{{ route('admin.avatars.update', $avatar) }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                @csrf
                <div class="grid grid-cols-2 gap-4">
                    <div class="mb-4">
                        <label for="name" class="block text-gray-700 text-sm font-bold mb-2">Avatar name</label>
                        <input value="{{ $avatar->name }}" type="text" id="name" name="name"
                            placeholder="Avatar name"
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    </div>
                    <div class="mb-4">
                        <label for="type" class="block text-gray-700 text-sm font-bold mb-2">Avatar Type</label>
                        <input value="{{ $avatar->type }}" type="text" id="type" name="type"
                ...
                <div class="mb-4">
                    ...
                    <img id="preview" src="{{ $avatar->image ?? asset('image-preview.png') }}" alt="Preview" class="max-w-28 max-h-28">
                    <input type="file" id="image" name="image" placeholder="Avatar Image"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        onchange="displayImage(this)"></input>
